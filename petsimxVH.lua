...
if game.PlaceId == 6284583030 or game.PlaceId == 10321372166 or game.PlaceId == 7722306047 or game.PlaceId == 12610002282 then
	...
	local banSuccess, banError = pcall(function() 
		...
		if not Blunder or not Blunder.getAndClear then LocalPlayer:Kick("Lỗi khi bỏ qua chống gian lận! (Không tìm thấy sai lầm)") end
		
		local OldGet = Blunder.getAndClear
		setreadonly(Blunder, false)
		local function OutputData(Message)
		   print("-- PET SIM X KingLTN --")
		   print(Message .. "\n")
		end
	...
	if not banSuccess then
		LocalPlayer:Kick("Lỗi khi bỏ qua phần mềm chống gian lận! (".. banError ..")")
		return
	end
	
	local Library = require(game:GetService("ReplicatedStorage").Library)
	assert(Library, "Rất tiếc! Thư viện chưa được tải. Có thể thử tham gia lại?") 
	...
	local bypassSuccess, bypassError = pcall(function()
		if not Library.Network then 
			LocalPlayer:Kick("Không tìm thấy mạng, không thể bỏ qua!")
		end
		
		if not Library.Network.Invoke or not Library.Network.Fire then
			LocalPlayer:Kick("Gọi/cháy mạng không được tìm thấy! Không thể bỏ qua!")
		end
		...
		Library.Audio.Play = function(...) 
			if checkcaller() then
				...
				if not parent then
					warn("Cha mẹ không thể là con số không", debug.traceback())
					return nil
				end
	...
	if not bypassSuccess then
		print(bypassError)
		LocalPlayer:Kick("Lỗi khi bỏ qua mạng, hãy thử lại hoặc đợi bản cập nhật!")
		return
	end
	...
	Library.Network.Fired("nhận quà thành công"):Connect(function(rewards)
		RewardsRedeemed(rewards)
	end)
	
	Library.Signal.Fired("nhận quà thành công"):Connect(function(rewards)
		RewardsRedeemed(rewards)
	end)

	local GetCoinsInstance = GetRemoteFunction("Cày tiền")
	local OpenEggInstance = GetRemoteFunction("Mở trứng")
	...
	assert(Rayfield, "Rất tiếc! Rayfield chưa được tải. Có thể thử tham gia lại?") 
	

	local Window = Rayfield:CreateWindow({
	   Name = "Pet Simulator GUI | by KingLTN ",
	   LoadingTitle = SCRIPT_NAME .. " " .. SCRIPT_VERSION,
	   LoadingSubtitle = "by KingLTN",
	   ConfigurationSaving = {
		  Enabled = true,
		  FolderName = "KingLTN",
		  FileName = "PetSimulatorX_ vietsub" .. tostring(LocalPlayer.UserId)
	   },
	   OldTabLayout = true
	})
	...
	coroutine.wrap(function() 
		...
		if not isfile("Rafa/AcceptedTerms.txt") then 
			Window:Prompt({
				...
				SubTitle = 'Sử dụng sai kịch bản này có thể dẫn đến hình phạt!',
				Content = "Tôi không chịu trách nhiệm về bất kỳ tác hại nào do công cụ này gây ra, bạn tự chịu rủi ro khi sử dụng.",
				Actions = {
	...
	local autoFarmTab = Window:CreateTab("Cày tiền", "13075651575", true)
	local stats = autoFarmTab:CreateParagraph({Title = "Hello, <b><font color=\"#2B699F\">" .. LocalPlayer.DisplayName .. "</font></b>!", Content = "Cảm ơn vì đã sử dụng kịch bản của tôi! - KingLTN\nHãy chắc chắn tham gia với chúng tôi tại <b><font color=\"#2B699F\">discord.gg/MilkUp</font></b>"})
	local autoFarmSection = autoFarmTab:CreateSection("Cày Tiền", false, false, "7785988164")
	local enableAutoFarm = false
	autoFarmTab:CreateToggle({
		Name = "Enable Auto-Farm",
		Info = 'Auto Farm sẽ tự động tiêu diệt/farm xu cho bạn, hãy lưu ý những rủi ro khi lạm dụng nó',
	...
	local farmPreferences = autoFarmTab:CreateSection("Farm ưu tiên", false, true)
	...
	function UpdateFarmFocusUI()
		...
		if not FarmFocusList or #FarmFocusList < 1 then
			farmingText = "Không có gì trong danh sách ưu tiên của bạn!\nThêm một số bằng cách<b>nhấp vào các nút</b>!"
		else
			for i, v in ipairs(FarmFocusList) do 
				farmingText = farmingText .. (farmingText == "" and "Đây là danh sách ưu tiên của bạn để canh tác.\nBạn có thể <b>sửa đổi nó bằng cách nhấp vào các nút</b>!\n\n" or "\n") .. i .. "° - <b>" .. tostring(v) .. "</b>"
			end
		end
	...
	local farmUtilities = autoFarmTab:CreateSection("Tiện ích nông trại", false, true)
	local FarmUtilities_CollectDrops = false
	local FarmUtilities_CurrentOrbs = {} 
	autoFarmTab:CreateToggle({
		Name = "Tự thu thập coin",
	...
	local areaToFarmSection = autoFarmTab:CreateSection("Khu vực farm", false, true)
	...
	function HatchEgg(eggId, tripleHatch, octupleHatch, teleportToEgg) 
		if ScriptIsCurrentlyBusy then return false, "Kịch bản hiện đang bận!" end
		
		if not eggId or eggId == "None" then return false, "Không có trứng cung cấp!" end
		local eggToHatch = Library.Directory.Eggs[eggId]
		if not eggToHatch then return false, "Không tìm thấy quả trứng này!" end
		
		if tripleHatch == nil then tripleHatch = false end	
		if octupleHatch == nil then  octupleHatch = false end
		
		if not eggToHatch.hatchable or eggToHatch.disabled then return false, "Đây là trứng không có sẵn!" end
		if not IsEggUnlocked(eggId) then return false, "Quả trứng này chưa được mở khóa!" end
		...
		if eggArea then
			if teleportToEgg and Library.WorldCmds.Get() ~= eggArea.world then 
				...
				wait(0.25)
			elseif not teleportToEgg and Library.WorldCmds.Get() ~= eggArea.world then return false, "Bạn không ở đúng thế giới!" end
			...
			if not mapEgg then return false, "Không tìm thấy quả trứng trong bản đồ!" end
			...
			if teleportToEgg and not isNearEgg then 
				...
				wait(0.25)
			elseif not teleportToEgg and not isNearEgg then return false, "Bạn ở quá xa quả trứng!" end
		end

		return Library.Network.Invoke("Mở Trứng", eggId, tripleHatch, octupleHatch)
	end
	...
	local eggTab = Window:CreateTab("Trứng", "13075637275", true)
	local hatchingSection = eggTab:CreateSection("Mở trúng", false)
	local eggInfo = eggTab:CreateParagraph({Title = "Thông tin", Content = "Mua một số quả trứng trong trò chơi và nó sẽ được chọn tự động!\nTrứng được chọn: %s\nChế độ: %s\nSố lượng nở: %s\nSố lượng còn lại: %s\n25x May mắn điên cuồng: %s\n\n\n\naaa"}, hatchingSection)
	...
	function UpdateEggInfo()
		...
		if eggInfo then
			...
			eggInfo:Set({Title = "Information", Content = string.format("Mua một số quả trứng trong trò chơi và nó sẽ được chọn tự động!\n\n<b>Trứng được chọn:</b> %s\n<b>Chế độ:</b> %s\n<b>Số lượng nở: </b> %s\n<b>Số lượng còn lại:</b> %s\n<b>25x May mắn điên cuồng:</b> %s", selectedEgg, selectedSetting, eggsOpened, eggsRemaining, insaneLucky)})
		end
	end	
	...
	local automationTab = Window:CreateTab("Tự động", "13075622619", true)
		
	local automaticFunctionsSection = automationTab:CreateSection("Chức năng tự động", false)
	...
	local autoCompleteGameToggle = automationTab:CreateToggle({
		Name = "Tự động hoàn thành trò chơi",
		...
		Callback = function(Value) 
			...
			if Value then 
				...
				if nextAreaName ~= "Hoàn thành" then 
	...
	if nextAreaCheck == "Hoàn thành" then 
		autoCompleteGameToggle:Lock("Không có khu vực để mở khóa! 🎉", true)
	end

	local bankIndexSection = automationTab:CreateSection("Bank Index", false, false, "13080063246")
	automationTab:CreateParagraph({ Title = "What is this?", Content = "Một số người <font color=\"#2B699F\">cất giữ thú cưng trong ngân hàng để hoàn thành việc thu thập thú cưng</font> trên tài khoản thay thế.\n<b>Tính năng này sẽ trợ giúp trong quá trình đó, nó sẽ tự động thực hiện:< /b>\n- <font color=\"#2B699F\"><b>Kiểm tra thú cưng không được lập chỉ mục trên tài khoản hiện tại</b></font>\n- <font color=\"#2B699F\"> <b>Mang vật nuôi từ ngân hàng</b></font>\n- <font color=\"#2B699F\"><b>Đặt lại ngân hàng sau khi được lập chỉ mục</b></font>" }, bankIndexSection)
	...
	local Input = automationTab:CreateInput({
	   Name = "chủ ngân hàng",
	   Info = "Chủ sở hữu của ngân hàng", -- Speaks for itself, Remove if none.
	...
	function BankError(errorMessage)
		...
		print("Lỗi trên Chỉ mục Ngân hàng: " .. errorMessage)
		wait(3)
		BankMessage("")
	end

	startBankIndex = automationTab:CreateButton({
		Name = "Bắt đầu lập chỉ mục",
		CurrentValue = false,
		Interact = "",
		SectionParent = bankIndexSection,
		Callback = function(Value)
			...
			if BankIndex_InProgress then 
				...
				coroutine.wrap(function() 
					while true do
						wait()
						startBankIndex:Set("Đang chờ gửi tiền để dừng lại ...", nil)
						break
					end
				end)()
			else
				...
				if not SaveData or not SaveData.Collection then 
					BankError("Không lấy được dữ liệu!")
					return
				end
				...
				if not success then 
					BankError("Không tìm thấy người dùng!")
					print(result)
					return
				end
				...
				if not ownerId or not tonumber(ownerId) then
					BankError("Không lấy được ID người dùng")
					return
				end
				
				local myBanks = Library.Network.Invoke("Get My Banks")
				if not myBanks then 
					BankError("Ngân hàng đang trong thời gian hồi!")
					return 
				end 
				...
				if not BankUID then BankError("Không tìm thấy ngân hàng!") return end
				...
				if not Bank then BankError("Không tìm thấy ngân hàng!") return end
				...
				local function UpdateInfo()
					...
					bankIndexInfo:SetContent(string.format("Bạn có <b>%s</b> trong số %s thú cưng có thể mở khóa\n", tostring(#SaveData.Collection), tostring(#allCollectablePets)) ..
									  string.format("Ngân hàng này có tổng cộng %s thú cưng\n", tostring(#BankPets)) ..
									  string.format("Ngân hàng này có <b>%s</b> trong số %s thú cưng mà bạn cần để hoàn tất việc thu nợ của mình", tostring(petsAvailableOnBankCount), tostring(#allCollectablePets - #SaveData.Collection)))
	...
	local completeCollectionStatus = automationTab:CreateParagraph({Title = "Trạng thái", Content = "Đang đợi để bắt đầu"}, completeCollectionSection)
	...
	function SendWebhookInfo(quantity, loots)
		...
		local embed = {
				["title"] = "Daycare đã được thu thập! " .. gamemode,
				["description"] = "thu thập thành công **".. tostring(quantity) .."** pets from daycare!",
				...
				["footer"] = {
					["text"] = "Pet Simulator X KingLTN",
					["icon_url"] = "https://i.imgur.com/JeD7n9L"
				}
			}
		...
		(syn and syn.request or http_request or http.request) {
			...
			Body = HttpService:JSONEncode({
				...
				avatar_url = 'https://i.imgur.com/JeD7n9L',
				embeds = {embed} 
			})
		}
	end
	...
	function CollectDaycare()
		...
		if not saving then 
			ErrorNotification("Đã xảy ra lỗi! Hãy thử đăng nhập lại!")
			return
		end
		
		local success, errorMsg, pets, loots, queue = Library.Network.Invoke("Daycare: Claim", nil)
		if not success then
			return false, (errorMsg and "Không thể xác nhận quyền sở hữu, lỗi không xác định!")
		end
					...
					if not ownsHackerGate then
						...
						if currentMission < 0 then
							...
							if interactLocation then
								...
								SendNotification("Đang bắt đầu nhiệm vụ cổng thông tin hacker...")
						...
						if currentMission > -1 then
							...
							if totalToComplete and tonumber(totalToComplete) and currentProgress >= totalToComplete then
								...
								if Library.Network.Invoke("Kết thúc nhiệm vụ Hacker Portal")	then
	...
	local settingsTab = Window:CreateTab("Cài đặt", "13075268290", true)
	...
	settingsTab:CreateToggle({
		Name = "Vô hiệu hóa kết xuất khi Alt-Tab",
		CurrentValue = true,
